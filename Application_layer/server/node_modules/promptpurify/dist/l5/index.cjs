...
var NON_LATIN_LETTER2 = /[\p{Script=Cyrillic}\p{Script=Greek}\p{Script=Armenian}\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}\p{Script=Arabic}\p{Script=Hebrew}]/u;
var ASCII_LETTER2 = /[A-Za-z]/;
...
function extractFeatures(text, firedRuleIds) {
  ...
  for (const tok of tokens) {
    if (NON_LATIN_LETTER2.test(tok)) nonLatinBearing++;
    else if (ASCII_LETTER2.test(tok)) asciiBearing++;
  }
  let upper = 0;
  let letters = 0;
  let punct = 0;
...
function defaultModelDir() {
  ...
  for (const rel of ["../../models/l5b", "../../../models/l5b"]) {
    ...
    if (fs.existsSync(path.join(p, "l5b.json"))) return p;
  }
  return path.join(here, "../../models/l5b");
}
...
function defaultModelDir2() {
  ...
  for (const rel of ["../../models/l5c", "../../../models/l5c"]) {
    const p = path.join(here, rel);
    if (fs.existsSync(path.join(p, "l5c.json"))) return p;
  }
  return path.join(here, "../../models/l5c");
}
...
async function createL5cRunner(opts = {}) {
  ...
  const contractPath = path.join(dir, "l5c.json");
  if (!fs.existsSync(contractPath))
    throw new L5bUnavailableError(
  ...
  const modelPath = path.join(dir, contract.model_file);
  const tokPath = path.join(dir, contract.tokenizer_file);
  ...
  const blob = JSON.parse(fs.readFileSync(tokPath, "utf8"));
...
function defaultModelDir3() {
  ...
  for (const rel of ["../../models/l5d", "../../../models/l5d"]) {
    const p = path.join(here, rel);
    if (fs.existsSync(path.join(p, "l5d.json"))) return p;
  }
  return path.join(here, "../../models/l5d");
}
...
async function createL5dRunner(opts = {}) {
  ...
  const contractPath = path.join(dir, "l5d.json");
  if (!fs.existsSync(contractPath))
    throw new L5bUnavailableError(
...
function defaultModelDir4() {
  ...
  for (const rel of ["../../models/l5e", "../../../models/l5e"]) {
    const p = path.join(here, rel);
    if (fs.existsSync(path.join(p, "l5e.json"))) return p;
  }
  return path.join(here, "../../models/l5e");
}
...
async function createL5eRunner(opts = {}) {
  ...
  const contractPath = path.join(dir, "l5e.json");
  if (!fs.existsSync(contractPath))
    throw new L5bUnavailableError(
...
exports.L5A_VERSION = L5A_VERSION;
exports.L5bUnavailableError = L5bUnavailableError;
exports._featurize = _featurize;
exports.createL5Cascade = createL5Cascade;
exports.createL5aClassifier = createL5aClassifier;
exports.createL5bRunner = createL5bRunner;
exports.createL5cRunner = createL5cRunner;
exports.createL5dRunner = createL5dRunner;
exports.createL5eRunner = createL5eRunner;
