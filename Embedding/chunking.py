...
def chunk_text(
    ...
    while start < len(text):
        ...
        if chunk:
            # Find the true start offset of the stripped chunk
...
def build_page_aware_chunks(
    ...
    for idx, (chunk_text_value, chunk_start) in enumerate(raw_chunks):
        # Drop garbage chunks (tables, charts, flattened figures)
        ...
        stable_id_input = f"{doc_id}|{source_json_key}|{len(chunks)}|{chunk_text_value}"
